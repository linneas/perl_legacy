...
# findClosestGeneFromGFFdetailed.pl
# written by Linnéa Smeds                       19 Aug 2015
# Modification that also list if the midpoint is up or down
# stream from the gene.
# =========================================================
...
while(<IN>) {
	...
	#		print STDERR "DEBUG: looking at scaff ".$tab[0]." ".$key." ".$hash{$tab[0]}{$key}{'end'}."\n";
			 if($hash{$tab[0]}{$key}{'stop'}<$tab[1]) {	#Check the closest gene to the left
			 	...
			 	if($hash{$tab[0]}{$key}{'strand'} eq "+") {
			 		$stream="downstream";
			 	}
			 	elsif($hash{$tab[0]}{$key}{'strand'} eq "-") {
			 		$stream="upstream";
			 	}
			 	else {
			 		die "ERROR - something is wrong for $gene, no strand information!\n";
			 	}
			 }
			 else {	
	...
	print join("\t", @tab)."\t".$dist."\t".$gene."\t".$stream."\n";
}
close(IN);
