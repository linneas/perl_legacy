...
#&trimEnds($read1, $preifx, $HQlim, $lowlim, $maxNoHQ, $maxNoLQ, $minReadLen, $scoring, 1);

if($read2) {
	unless(-e $read2) {
		die "Error: File $read2 doesn't exist!\n";
	}
#	&trimEnds($read2, $prefix, $HQlim, $lowlim, $maxNoHQ, $maxNoLQ, $minReadLen, $scoring, 2);
...
sub trimEnds { 
	...
	open(IN, $reads);
	my $out = $outpref."_temp" . $pairNo . ".fastq";
	open(OUT, ">$out");
...
sub removeBadPairs {
	...
	open(TR1, "$outpref"."_temp1.fastq");
	open(TR2, "$outpref"."_temp2.fastq");
	open(OUT1, ">$out1");
	open(OUT2, ">$out2");
	open(OUT3, ">$out3");
	...
	system("rm $outpref"."_temp1.fastq $outpref"."_temp2.fastq");
	$curr_time = time-$curr_time;
	if($curr_time<60) {
		print "\tdone! ($curr_time sec)\n";
	}
...
sub removeBadSingle {
	...
	system("rm $outpref"."_temp1.fastq");
	$curr_time = time-$curr_time;
	if($curr_time<60) {
		print "\tdone! ($curr_time sec)\n";
	}
