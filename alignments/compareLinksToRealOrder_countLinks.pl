...
# compareLinksToRealOrder_countLinks.pl
# written by Linnéa Smeds                  	15 May 2012
# modified to count Links on 4 June 2012
# =========================================================
...
my %links = ();	
print "open file $linkFile\n";	
open(IN, $linkFile);
while(<IN>) {
	my @tab = split(/\s+/, $_);
	my ($startNo, $startDir, $dum1, $startLinks) = split(/[()]/, $tab[1]);
	my ($endNo, $endDir, $dum2, $endLinks) = split(/[()]/, $tab[2]);
	...
	$links{$tab[0]}{'+'}{'links'}=$startLinks;
	$links{$tab[0]}{'-'}{'name'}=$endNo;
	$links{$tab[0]}{'-'}{'dir'}=$endDir;
	$links{$tab[0]}{'-'}{'signif'}=$endSignif;
	$links{$tab[0]}{'-'}{'links'}=$endLinks;
		print "Have saved ".$tab[0]." with $startNo, $startDir, $startSignif and $startLinks\n";
}
close(IN);
		...
		while($chr eq $chrom) {
			...
			while($check eq "ok") {
				...
				if(defined $knownScaffs{$nextScaf}) {
					...
					if(defined $knownScaffs{$lastScaf} && $knownScaffs{$nextScaf}) {
						print BAD $lastScaf."(".$links{$lastScaf}{$lastDir}{'links'}.")\t".
								$knownScaffs{$lastScaf}."\t".$nextScaf."(".
								$links{$nextScaf}{&oppDir($nextDir)}{'links'}.")\t".
								$knownScaffs{$nextScaf}."\n";
					}	
					last;
				}
			...
			while($check eq "ok") {
				if($nextScaf eq $nexttab[1]) {
					...
					$tempOrder = $tempOrder."\n".$nextScaf."(".$nextDir.")";
					print "Temporder is now : $tempOrder\n";
					print GOOD $chr."\t".$lastScaf."(".$links{$lastScaf}{$lastDir}{'links'}.")\t".
							$nextScaf."(".$links{$nextScaf}{&oppDir($nextDir)}{'links'}.")\n";
					$printFlag = "off";
					last;
				}
				if(defined $knownScaffs{$nextScaf}) {
	...
	if(defined $links{$scaff}) {
		...
		if(defined $links{$next}) {
			if($links{$next}{$nextKey}{'name'} eq $scaff) {
				print "$scaff $key is attached to $next $nextKey with ".
					$links{$scaff}{$key}{'links'}." - links from both sides\n";
		...
		#		print "return from sub in sub; $test\n";	
				...
				print "$scaff $key is attached to $next $nextKey with ".
					$links{$scaff}{$key}{'links'}." - links from this side only\n";
			}
		}
		else {
