...
while(<ALL>) {
	...
	system("awk '(\$3==\"$scaff\" && \$4<$limit){print}' $sam |cut -f7 |sort |uniq -c |sort -nr >$temp");
...
#		print "adding $Name and $No to the array\n";
	...
	my $top = $NameARR[0];
	my $toStart = `awk '(\$3==\"$scaff\" && \$4<$limit && \$7==\"$top\" && \$8<$limit){print}' $sam |wc -l`;
	chomp($toStart);
	my $toEnd = `awk '(\$3==\"$scaff\" && \$4<$limit && \$7==\"$top\" && \$8>$limit){print}' $sam |wc -l`;
	chomp($toEnd);
	...
	system("awk '(\$3==\"$scaff\" && \$4>$tempEnd){print}' $sam |cut -f7 |sort |uniq -c |sort -nr >$temp");
...
#		print "adding $Name and $No to the array\n";
	...
	# Check if the links are in the start/end of the new scaffold
	$top = $NameARR[0];
	$toStart = `awk '(\$3==\"$scaff\" && \$4>$limit && \$7==\"$top\" && \$8<$limit){print}' $sam |wc -l`;
	chomp($toStart);
	$toEnd = `awk '(\$3==\"$scaff\" && \$4>$limit && \$7==\"$top\" && \$8>$limit){print}' $sam |wc -l`;
	chomp($toEnd);
