...
# # # # # #
# rearrLengthFromMGR_addChromLen.pl
# written by Linnéa Smeds                    Jan 2013
# UPDATE: This version was written on 14 Feb 2013 to
# also make a list with rearrangement lengths together
# with chromosome lengths.
# ===================================================
...
# Input parameters
my $CHROMLIST = $ARGV[0];
my $KARYOLEN = $ARGV[1];	# KaryoID (eg "fa1") anf length
my $SP_NO = $ARGV[2];	#Depending on how sp are sorted,
...
my $WITHCHRLEN = $OUTPREF.".rearrAndChromLen.txt";
open(OUT3, ">$WITHCHRLEN");
print OUT3 "#ID	CHR_LEN	REARR_LEN\n";
...
#			print "DEBUG: with blocks: $concat\n";
			...
			print OUT3 $blocks{$from}{'id'}."\t".$karlen{$blocks{$from}{'id'}}."\t".$sum."\n";
