...
# condetri_v2.3.pl
# September 2010, mod Oct 2011, Mar, Jun & Oct 2012
...
#Checking Zlib
my $check = `perl -MIO::Zlib -e 1 |wc -w`;
chomp($check);
if($check ne "0") {
	die "Error: IO::Zlib is not installed! \n".
		"Try installing it with sudo perl -MCPAN -e 'install \"IO::Zlib\"'\n";
}
use IO::Zlib;
