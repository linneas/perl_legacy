...
# # # # # #
# concatFromAnchors_PIPEpart1_removeQueryDupl_v2.pl
...
#	print "remove overlaps for $chrom\n";
	...
	foreach my $line (@rows) {
		...
		if(defined $hash{$tabs[0]}{$tabs[1]}) {
			...
			"\t".$hash{$tabs[0]}{$tabs[1]}{'querdir'}."\t".$hash{$tabs[0]}{$tabs[1]}{'score'}."\n";
...
sub removeQueryOverlaps {
	...
	foreach my $line (@rows) {
		...
		if(defined $hash{$tabs[4]}{$tabs[5]}) {
			...
			$hash{$tabs[4]}{$tabs[5]}{'dir'}."\t".$hash{$tabs[4]}{$tabs[5]}{'score'}."\n";
...
sub makeConcatAnchorFile {
	...
	system("cat *hr*_step$steps.anchors >$concatAnch");
}
