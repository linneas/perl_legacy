...
#	print "remove overlaps for $chrom\n";
	...
	my $outfile = $chrom.".noovl.anchors";
...
sub allMergeSteps {
	...
	while(my $line = <CNF>) {
		...
		if($line =~ m/^#/ || $line eq "") {
			...
			$outStep = $chrom.".step".$cnt.".anchors";
			if($type eq "M") {
