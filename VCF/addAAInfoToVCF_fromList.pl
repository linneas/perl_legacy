...
# Modified from addAAInfoToVCF.pl which takes ancestral
# alleles from a fasta file.
# =====================================================
...
while(<LIST>) {
	if($acnt==0) {
		unless($_ =~ m/\w+:\d+\s[ATCGN]/) {
			die "File with ancestral allele does not have the format SEQ:POS ANC_ALLELE\n but looks like this: $_";
		}
	}
	my ($id,$anc)=split(/\s+/, $_);
	$hash{$id}=$anc;
