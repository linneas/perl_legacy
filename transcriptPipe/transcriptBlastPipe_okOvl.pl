...
# Files
my $querySeq = "$querySpecies/$queryFile";
...
&preparation($queryFile, $querySpecies, $targetSeq, $formatedTar, $formatedQuery, $mappingQuery);
...
&saveFoundGenes($queryFile, $querySpecies,$HitSeqOut, $BlastOut, $tmpRecHitList, $FullOutput, $FullMapTable);
...
sub mergeOverlap {
	...
	my ($alignOverlap, $otherOverlap, $fullOverlap) = (0,0,0);
	my $badFlag = "off";
	my $cnt = 0;
	open(IN, $FullMap);
	open(OUT, ">$Output");
						...
						if($currentEnd-$a[1]<=$Kmer) {
							...
							if($RealAlignLen<=$Kmer) {
								...
								if($RealAlignLen>length($tempSeq)){
									print STDERR "Warning in gene $currentGene: contig ". $a[3].
										" is shorter than overlap and cannot be used!\n".
	...
	print "...$fullOverlap contigs overlapped with their full length and could not be used.\n";
