...
if(merge==FALSE) {
	...
	# Open outfile	
	if(pdf.out==TRUE) {
		outfile <- paste("chrom",chrom,"_GCcov_sep_",windSize,"kb_windows.pdf",sep="")
		wid<-11*scale
		pdf(outfile,width=wid,height=5.5)
		headPos<-11
	}else {
		outfile <- paste("chrom",chrom,"_GCcov_sep_",windSize,"kb_windows.png",sep="")
		wid<-1000*scale
	...
	#Starting with G+C
	plot(1,1, type="n", xaxt="n", ylab="G+C", xlim=c(0,xlen), ylim=c(0,ylen1))
	xstart <- 1
	...
	#Adding scale
	segments(1, ylen1*0.8, scaleMark+1, ylen1*0.8)
	segments(1, ylen1*0.82, 1, ylen1*0.78)
	segments(scaleMark+1, ylen1*0.82, scaleMark+1, ylen1*0.78)
	text(scaleMark/2+1, ylen1*0.8, labels=scaleMark.name, cex=0.8, pos=3)

	#Plotting coverage below, and the scaffold segment at bottom
	par(mar=c(5,3,0.2,3))
	par(xpd=TRUE)
	plot(1, 1, type="n", xaxt="n", ylab="Coverage", xlab="", xlim=c(0,xlen), ylim=c(0,ylen2))
	xstart <- 1
	for (i in 1:length(LST\$V1)) {
	#	show(xstart)
	...
	#		show(xstart)
	#		show(length(xval))
			segments(xstart, -ylen2*0.1, length(yval)+xstart-1, -ylen2*0.1)
			segments(xstart, -ylen2*0.15, length(yval)+xstart-1, -ylen2*0.15)
			segments(xstart, -ylen2*0.1, xstart, -ylen2*0.15)
			segments(length(yval)+xstart-1, -ylen2*0.1, length(yval)+xstart-1, -ylen2*0.15)
		}else{ 
			segments(xstart, -ylen2*0.1, xstart, -ylen2*0.15)
		}
		text(xstart+length(yval)/2, -ylen2*0.25, labels=levels(factor(LST\$V1[i])), pos=1, srt=90, xpd=TRUE, cex=0.7) 
		xstart<-xstart+length(yval)+gapsize

	}

	#Adding scale
	segments(1, ylen2*0.8, scaleMark+1, ylen2*0.8)
	segments(1, ylen2*0.82, 1, ylen2*0.78)
	segments(scaleMark+1, ylen2*0.82, scaleMark+1, ylen2*0.78)
	text(scaleMark/2+1, ylen2*0.8, labels=scaleMark.name, cex=0.8, pos=3)
...
##################################################################################################	
	...
	###############################################################################################
	### G+C and coverage in merged plot

	#Starting with G+C
	plot(1,1, type="n", xaxt="n", ylab="G+C", xlab="", xlim=c(0,xlen), ylim=c(0,ylen1))
	xstart <- 1
	for (i in 1:length(LST\$V1)) {
	#	show(xstart)
		yval<-TB\$V5[TB\$V1==levels(factor(LST\$V1[i]))]
	
		if(length(yval)>0) {
			xval<-seq(xstart,length(yval)+xstart-1, 1)
	#		show(xval)
			lines(xval, yval, type="l", col="red")
		} 
		xstart<-xstart+length(yval)+gapsize
	}
	...
	#Reset the y-axis
	par("usr")
	par(usr=c(par("usr")[1:2], 0, ylen2))
	xstart <- 1
	for (i in 1:length(LST\$V1)) {
	#	show(xstart)
		yval<-TB\$V4[TB\$V1==levels(factor(LST\$V1[i]))]
		if(length(yval)>0) {
			xval<-seq(xstart,length(yval)+xstart-1, 1)
	#		show(xval)
			lines(xval, yval, type="l", col="gray")
			segments(xstart, ylen2*0.05, length(yval)+xstart-1, ylen2*0.05)
			segments(xstart, ylen2*0.1, length(yval)+xstart-1, ylen2*0.1)
			segments(xstart, ylen2*0.05, xstart, ylen2*0.1)
			segments(length(yval)+xstart-1, ylen2*0.05, length(yval)+xstart-1, ylen2*0.1)
		}else{ 
			segments(xstart, ylen2*0.05, xstart, ylen2*0.1)
		}
		text(xstart+length(yval)/2, -ylen2*0.15, labels=levels(factor(LST\$V1[i])), pos=1, srt=90, xpd=TRUE, cex=0.7) 
		xstart<-xstart+length(yval)+gapsize
	}
	...
	axis(4, at=seq(0,120,20), labels=seq(0,ylen2,20))
	mtext("Coverage", side=4, line=1, cex=0.8, las=0)
	mtext(title, side=3, line=headPos, cex=1.2, las=0)
	legend("topright", inset=0.01, c("G+C", "Coverage"), col=c("red", "gray"), lty=1, cex=0.6, bty="n")
