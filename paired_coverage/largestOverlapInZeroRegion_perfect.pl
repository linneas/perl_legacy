...
# largestOverlapInZeroRegion_perfect.pl
# written by Linnéa Smeds                       August 2011
# edited on November 13, 2013
# =========================================================
# Takes a list of zero regions and finds the largest
# possible overlap among left-side pairs and right-side
# pairs.
# THIS VERSION ONLY LOOKS AT READS WITHOUT INDELS AND MIS-
# MATCHES (SOFTMASKES ARE OK)
# =========================================================
...
#	print "looking at $scaff with start $start and end $end and creating tempfile $temp\n";
	...
	system("samtools view -X -f 0x0002 $bam $scaff:$checkSt-$checkEnd |awk '(\$7==\"=\"){print}' |grep \"NM:i:0\"  >$temp");
	...
	foreach my $keys (keys %sams) {
		if(defined $sams{$keys}{'left'} && defined $sams{$keys}{'right'}) {	
			...
			if($pairEnd<=$start) {
				if($sams{$keys}{'right'}{'maplen'}+$sams{$keys}{'right'}{'start'}-1 >$maxLeftSide) { #&& $sams{$keys}{'right'}{'maplen'}+$sams{$keys}{'right'}{'start'}-1 > $start) { # Removed this because left and right doesn't have to overlap!
	...
	if($minRightSide > $end) {
		print "WARNING: right side starts at $minRightSide which is after end ($end)\n";
		if($minRightSide==100000000) {
			$minRightSide = $end; #changed from $start;
		}
	}
	if($maxLeftSide == 0) {
		print "WARNING: left side ends at $maxLeftSide which is before start ($start)\n";
		if($maxLeftSide==0) {
			$maxLeftSide = $start; #changed from $end;
		}
	}
